...
namespace igloo15.MarkdownApi.Core.Builders
{
    ...
    internal static class VSDocParser
    {
        ...
        {
            var typeNameMatch = Regex.Match(m.Value, "\"[^\"]*\"");
            var typeName = typeNameMatch.Groups[0].Value.Replace("\"", string.Empty);
            ...
            var displayedName = $"[{typeName.Split('.').Last()}]";
            var baseUrl = "(https://github.com/sigstat/sigstat/blob/develop/docs/md/";
            string itemPath;
            if (typeName.StartsWith("F") || typeName.StartsWith("P") || typeName.StartsWith("E"))
            {
                itemPath = typeName.Replace('.', '/').Substring(typeName.IndexOf(":") + 1) + ")";
                itemPath = itemPath.Substring(0, itemPath.LastIndexOf("/")) + ".md)";
                returned = displayedName + baseUrl + itemPath;
            }
            else
            {
                itemPath = typeName.Replace('.', '/').Substring(typeName.IndexOf(":") + 1) + ".md)";
                returned = displayedName + baseUrl + itemPath;
            }
